...
rule bowtie2:
    ...
    resources:
        ...
        time="01:00:00",
    shell: """
        bowtie2 -1 {input.r1} -2 {input.r2} -x ../results/03-aDNA_authentication/indexes/{wildcards.sample}_index/{wildcards.sample}.index -p {threads} | samtools view -@ {threads} -Sb | samtools sort -@ {threads} > {output.bam}
...
rule pydamage:
    input:
        "../results/03-aDNA_authentication/bowtie2_out/{sample}.bam"
    output:
        outdir=directory("../results/03-aDNA_authentication/pydamage/{sample}_pydamage"),
        pydamage="../results/03-aDNA_authentication/pydamage/{sample}_pydamage/pydamage_results.csv",
        pydamage_filtered="../results/03-aDNA_authentication/pydamage/{sample}_pydamage/{sample}_pydamage_filtered_results.csv",
    threads: 24
    log:
...
rule pydamage2fasta:
    input:
        pydamage_csv="../results/03-aDNA_authentication/pydamage/{sample}_pydamage/{sample}_pydamage_filtered_results.csv",
        viruses_fasta="../results/02-assembly/{sample}/{sample}_scaffolds_filtered_4k_20cov.fasta",
    output:
        ancient_viruses_fasta="../results/03-aDNA_authentication/ancient_contigs_fasta/{sample}_ancient.fasta",
    params:
        all_pydamage_outputs=directory("../results/03-aDNA_authentication/pydamage/pydamage_all_outputs"),
        sample_name="{sample}"
    threads: 1
    log:
